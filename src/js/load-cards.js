(function (carusel) {
  ...
  const loadNews = () => {
    ...
    new Promise((resolve, reject) => {
      ...
      xhr.open("GET", "https://cors-anywhere.herokuapp.com/http://api.lenta.ru/lists/latest", true);
  ...
  const addTime = date => {
    ...
    return !!date
      ? `<time class="br-card__item-pd br-card__date">${date.toLocaleString("ru", options)}</time>`
      : "";
  };
